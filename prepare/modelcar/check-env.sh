#!/usr/bin/env bash
#
# check-env.sh — Verify the cluster is ready for the ModelCar OCI exercise.
#
...
#   3.  OpenShift version
#   4.  RHOAI 3.4 installed and DataScienceCluster Ready
#   5.  GPU availability (at least one nvidia.com/gpu node)
#   6.  Required CLI tools (oc, podman, curl, git, python3)
#   7.  vLLM ServingRuntime template (created by RHOAI)
#   8.  GPU hardware profile (gpu-profile)
#   9.  Internal image registry is Managed (can be exposed for pushes)
...
# --- Exercise constants -------------------------------------------------------
LAB_NAMESPACE="prepare-modelcar"
RHOAI_NS="redhat-ods-applications"
RUNTIME_TEMPLATE="vllm-cuda-runtime-template"
HW_PROFILE="gpu-profile"
MIN_RHOAI_VERSION="3.4"
IMAGESTREAM="modelcar-qwen3-06b"

echo ""
echo -e "${BOLD}ModelCar OCI Exercise — Environment Check${NC}"
echo "Verifying platform prerequisites for building and deploying a ModelCar image"
...
if oc auth can-i '*' '*' --all-namespaces &>/dev/null; then
  ...
  echo -e "     ${CYAN}Why:${NC} The exercise setup exposes the internal registry (a cluster-scoped change) and tags images"
...
get_tool_version() {
  case "$1" in
    ...
    podman)  podman version --format '{{.Client.Version}}' 2>/dev/null ;;
...
# not fooled by the `oc` shell function defined above.
...
# Podman is used to build and push the ModelCar image. If it is unavailable,
# the exercise offers a fallback (pre-imported image), so this is a warning.
if type -P podman &>/dev/null; then
  check_pass "podman: $(get_tool_version podman)"
  if podman info &>/dev/null; then
    check_pass "podman engine is reachable"
  else
    check_warn "podman is installed but the engine is not reachable"
    echo -e "     ${CYAN}Info:${NC} You can still complete the exercise using the fallback pre-imported image."
    echo -e "     ${CYAN}Fix:${NC} On macOS, run: podman machine init && podman machine start"
  fi
else
  check_warn "podman not found in PATH"
  echo -e "     ${CYAN}Info:${NC} Needed to build/push the image. Without it, use the fallback pre-imported image."
fi
...
if [[ "$REG_STATE" == "Managed" ]]; then
  check_pass "Internal image registry is Managed (can be exposed for pushes)"
else
  check_fail "Internal image registry managementState is '${REG_STATE:-unknown}' (expected Managed)"
  echo -e "     ${CYAN}Why:${NC} The exercise pushes the ModelCar image to the internal registry"
...
REG_ROUTE=$(oc get route default-route -n openshift-image-registry -o jsonpath='{.spec.host}' 2>/dev/null || echo "")
if [[ -n "$REG_ROUTE" ]]; then
  check_pass "Registry external route is exposed: $REG_ROUTE"
else
  info "Registry external route not exposed yet — the setup step enables it (spec.defaultRoute: true)"
fi
...
if oc get namespace "$LAB_NAMESPACE" &>/dev/null; then
  ...
  echo -e "     ${CYAN}Fix:${NC} oc delete project $LAB_NAMESPACE   (wait for full deletion before re-running setup)"
  ((LEFTOVERS++))

  if oc get imagestream "$IMAGESTREAM" -n "$LAB_NAMESPACE" &>/dev/null; then
    check_warn "ImageStream '$IMAGESTREAM' exists in $LAB_NAMESPACE"
    ((LEFTOVERS++))
  fi
  if oc get inferenceservice qwen3-modelcar -n "$LAB_NAMESPACE" &>/dev/null; then
    check_warn "InferenceService 'qwen3-modelcar' exists in $LAB_NAMESPACE"
    ((LEFTOVERS++))
  fi
fi
...
if [[ "$FAIL" -eq 0 && "$WARN" -eq 0 ]]; then
  echo -e "${GREEN}${BOLD}Environment ready for the ModelCar exercise.${NC}"
elif [[ "$FAIL" -eq 0 ]]; then
