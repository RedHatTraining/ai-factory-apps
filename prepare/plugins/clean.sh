#!/usr/bin/env bash
#
# clean.sh — Reset the cluster to a known-good state for the vLLM plugins exercise.
#
...
#
# What it reverses:
#   1. The exercise namespace 'prepare-plugins' and everything in it: the
#      standard ServingRuntime, the imported imagestream tags, and the student's
#      custom ServingRuntime, InferenceService, and route.
#
# What it deliberately does NOT touch:
#   - The image registry managementState (stays Managed — required by check-env)
#   - The GPU node's image cache (the pre-warmed images are reused on the next
#     setup, making the next run faster; nothing here depends on a cold node)
#   - RHOAI, KServe, GPU operator, or any other platform component
#
# Note: setup.sh creates no external registry route and no local Podman images
# (students consume pre-built images), so there is nothing of that kind to undo.
#
...
echo ""
echo -e "${BOLD}vLLM Plugins Exercise — Clean / Reset${NC}"
...
# Deleting the namespace removes everything the exercise created: the standard
# and custom ServingRuntimes, the imported imagestream tags, the student's
# InferenceService, and its route.
