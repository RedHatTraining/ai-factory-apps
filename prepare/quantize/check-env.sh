#!/usr/bin/env bash
#
# check-env.sh — Verify the cluster is ready for the Quantize exercise.
#
...
# Podman is used to build and push the Quantize image. If it is unavailable,
...
if [[ "$REG_STATE" == "Managed" ]]; then
  ...
  echo -e "     ${CYAN}Why:${NC} The exercise pushes the Quantize image to the internal registry"
...
if [[ "$FAIL" -eq 0 && "$WARN" -eq 0 ]]; then
  echo -e "${GREEN}${BOLD}Environment ready for the Quantize exercise.${NC}"
elif [[ "$FAIL" -eq 0 ]]; then
